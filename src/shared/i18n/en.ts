const en = {
  app: {
    name: 'Youtive',
  },
  nav: {
    home: 'Home',
    library: 'Library',
  },
  language: {
    en: 'English',
    th: 'ไทย',
  },
  home: {
    titleLine1: 'Know what you\'ll get',
    titleLine2: 'before watching.',
    subtitle: 'Paste a YouTube video and understand its real value in 2 minutes.',
    step1Title: 'Paste any YouTube URL',
    step1Desc: 'No account needed. Just paste and go.',
    step2Title: 'See what you\'ll get',
    step2Desc: 'Understand the real outcomes before watching.',
    step3Title: 'Keep for later',
    step3Desc: 'Save it to your library and come back anytime.',
    exampleLabel: 'Try an example',
    inputPlaceholder: 'Paste YouTube URL here...',
    submit: 'Analyze',
    invalidUrl: 'Please enter a valid YouTube link',
    analyzing: 'Analyzing...',
  },
  library: {
    title: 'Your kept videos',
    subtitle: 'Saved for later, ready when you are.',
    count: '{{count}} saved',
  },
  result: {
    loading: 'Analyzing video...',
    error: 'Could not analyze this video.',
    retry: 'Try another URL',
    openOnYoutube: 'Open on YouTube',
  },
  outcome: {
    title: 'After this video, you\'ll be able to…',
    subtitle: 'These are the concrete outcomes you can expect from watching.',
  },
  summary: {
    fullSummary: 'Full Summary',
    hide: 'Hide',
    read: 'Read full summary',
    bigIdea: 'Big Idea',
    keyPoints: 'Key Points',
    usefulExamples: 'Useful Examples',
    thingsToRemember: 'Things to Remember',
  },
  keep: {
    title: 'Keep for later',
    desc: 'Save this to your library to revisit anytime.',
    button: 'Keep It',
    kept: 'Kept',
    toast: 'Saved to your library',
  },
  empty: {
    title: 'You haven\'t kept anything yet.',
    subtitle: 'Analyze a video and save it here to come back later.',
    action: 'Analyze a video',
  },
  card: {
    openSummary: 'Open summary',
    remove: 'Remove',
  },
  player: {
    playing: 'Playing',
    previous: 'Previous',
    next: 'Next',
    play: 'Play',
    pause: 'Pause',
  },
  rail: {
    seeAll: 'See All',
  },
  search: {
    placeholder: 'Search',
    findPodcast: 'Find a podcast...',
  },
  sidebar: {
    library: 'Library',
  },
};

export default en;
